...
resume=false        		# Resume the training from snapshot
train_lm=false
...
#feat_val_dir=${dumpdir}/al/delta${do_delta}; mkdir -p ${feat_val_dir}
#if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
	...
	mv avsrlrs3/exp/train_rnnlm_pytorch_lm_unigram500 exp/pretrainedlm
	mv avsrlrs3/data/lang_char data/
